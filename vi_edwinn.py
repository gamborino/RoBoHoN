...
all_actions = ['a100', 'a101', 'a102', 'a110', 'a120', 'a200', 'a310', 'a320']
#Preference
#pref = [0.0517, 0.0517, 0.2069, 0.2759, 0.023, 0.023, 0.3448, 0.023]
pref = [0.0, 0.0, 0.0, 0.4, 0.0, 0.0, 0.6, 0.0]
top2pref = [3, 6]
...
class robohon:
    def __init__(self, total_steps):
        ...
        self.ai = irl.sarsa(actions=range(n_actions), epsilon=1.0, alpha=0.3, gamma=0.9)
        ...
        self.p_action = np.random.choice([self.p_action1, self.p_action2], p=[0.2, 0.8])
        #self.p_action = self.p_action1
        self.epi_reward = 0
        self.count = 0
        self.total_steps = total_steps
        
    def update(self, actionid):
        self.ai.epsilon = (self.total_steps-len(self.reward)*5)/float(self.total_steps) * self.ai.epsilon
	...
	r_rs = self.get_reward_rs(self.p_action1, top2pref)
        #r_rs = 0.0
	self.epi_reward = r_rs + r_ps + self.epi_reward

        n_action1 = self.ai.chooseAction(n_IFV)
        
	if self.p_action is not None and self.p_IFV is not None:
	    self.ai.learn(self.p_IFV, self.p_action, r_ps, r_rs, n_IFV, n_action1)        
	self.p_IFV = n_IFV

        n_action2 = self.actionid_to_sarsa(actionid)
        n_action = np.random.choice([n_action1, n_action2], p=[0.2, 0.8])
        #n_action = n_action1
    ...
    def get_reward_rs(self, sarsa_action, preference):
...
for i in range(total_steps):
    actionid = np.random.choice(all_actions, p=pref)
    robot.update(actionid)
robot.save_reward_q()
